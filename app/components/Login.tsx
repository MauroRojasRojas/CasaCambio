...
export default function Login() {
	...
	return (
		<main
			className={`min-h-screen w-full bg-[#C7C0BD] flex items-center justify-center transition-opacity duration-600 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}
		>
			{/* Overlay de carga */}
			{isLoading && (
			...
			<div
				className={`bg-white w-[90%] max-w-6xl rounded-3xl shadow-xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 transition-all duration-500 ${isLoading ? 'scale-95 opacity-50' : 'scale-100 opacity-100'}`}
			>
				{/* ========================= */}
				{/*   COLUMNA IZQUIERDA       */}
				{/* ========================= */}
				<div className='px-10 py-14 flex flex-col justify-center'>
					...
					<p className='mt-2 text-sm text-slate-600'>
						...
						<a href='/register' className='text-[#0053A4] font-semibold hover:underline cursor-pointer'>
							{' '}
							Regístrate →
						</a>
					</p>
					...
					<div className='mt-8 space-y-5'>
						...
						<div>
							...
							<div className='relative mt-1'>
								...
								<span
									...
									className={`absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer text-slate-500 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
								>
						...
						{/* ERROR MESSAGE */}
						{loginError && <div className='text-red-600 text-sm bg-red-50 p-3 rounded-lg animate-in fade-in'>{loginError}</div>}
						...
						<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mt-1'>
							...
							<a
								href='/recuperar'
								className={`text-sm text-[#0053A4] hover:underline font-semibold cursor-pointer ${isLoading ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''}`}
							>
						...
						<button
							disabled={isLoading}
							className={`w-full mt-3 border border-slate-300 text-slate-700 py-3 rounded-xl font-semibold shadow-sm hover:bg-slate-50 transition-all text-sm flex items-center justify-center gap-3 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
						>
				...
				<div className='hidden lg:flex items-center justify-center bg-white'>
					<Image src='/assets/loggin.svg' alt='Login banner' width={600} height={600} className='object-contain p-3 rounded-xl' />
				</div>
			</div>
		</main>
	);
