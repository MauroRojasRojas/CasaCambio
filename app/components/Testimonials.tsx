...
export default function Testimonios() {
	...
	useEffect(() => {
		...
		const animate = (timestamp: number) => {
			...
			setTranslateX((prev) => {
				const totalWidth = testimonios.length * getCardWidth();
	...
	return (
		<section className='py-12 md:py-16 lg:py-20 4k:py-24 bg-yellow-200 overflow-hidden'>
			...
			<div className='relative' ref={containerRef}>
				...
				<div className='absolute left-0 top-0 bottom-0 w-16 sm:w-24 md:w-32 bg-gradient-to-r from-yellow-200 to-transparent z-10 pointer-events-none' />
				<div className='absolute right-0 top-0 bottom-0 w-16 sm:w-24 md:w-32 bg-gradient-to-l from-yellow-200 to-transparent z-10 pointer-events-none' />
				...
				>
					{duplicatedTestimonios.map((testimonio, idx) => (
						<div
							key={`${testimonio.id}-${idx}`}
							onClick={() => handleCardClick(idx, testimonio.id)}
							className={`bg-white rounded-2xl p-4 sm:p-5 md:p-6 shadow-lg cursor-pointer hover:shadow-xl transition-all duration-300 w-[280px] sm:w-[320px] md:w-[380px] flex-shrink-0 ${
								selectedId === testimonio.id ? 'ring-4 ring-[#02254A] scale-105' : ''
							}`}
						>
							<div className='text-red-600 text-2xl sm:text-3xl md:text-4xl mb-1'>"</div>

							<p className='text-slate-700 leading-relaxed mb-4 text-xs sm:text-sm line-clamp-4'>{testimonio.texto}</p>

							<div className='flex items-center gap-3'>
								<Image src={testimonio.foto} width={48} height={48} className='rounded-full object-cover shadow w-10 h-10 sm:w-12 sm:h-12' alt={testimonio.nombre} />
								<span className='font-bold text-[#02254A] text-sm sm:text-base'>{testimonio.nombre}</span>
							</div>
						</div>
					))}
				</div>
