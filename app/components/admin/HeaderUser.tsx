	...
	return (
		<header className='w-full bg-white h-16 shadow-sm px-4 sm:px-10 flex items-center justify-between border-b border-slate-200'>
			...
			<div className='flex items-center gap-5 relative'>
				...
				{/* DIALOG MENU */}
				{showMenu && (
					<div className='absolute right-0 top-12 bg-white border border-slate-200 rounded-lg shadow-lg min-w-max z-50'>
						...
						<Link href='/perfil'>
							<button className='w-full text-left px-6 py-4 hover:bg-slate-100 transition-colors text-[#02254A] text-base font-medium'>👤 Ver perfil</button>
						</Link>
						<button
							onClick={handleLogout}
							className='w-full text-left px-6 py-4 hover:bg-red-50 transition-colors text-red-600 text-base font-medium border-t border-slate-200'
						>
							🚪 Cerrar sesión
						</button>
					</div>
